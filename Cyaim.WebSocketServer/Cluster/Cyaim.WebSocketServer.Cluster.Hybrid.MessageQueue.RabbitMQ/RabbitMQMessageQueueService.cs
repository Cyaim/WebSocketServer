...
namespace Cyaim.WebSocketServer.Cluster.Hybrid.MessageQueue.RabbitMQ
{
    ...
    public class RabbitMQMessageQueueService : IMessageQueueService
    {
        ...
        public async Task ConnectAsync()
        {
            ...
            {
                _logger.LogWarning($"[RabbitMQMessageQueueService] RabbitMQ 已连接，跳过连接 - ConnectionIsOpen: {_connection.IsOpen}, ChannelIsOpen: {_channel.IsOpen}");
                return;
            }
            ...
            try
            {
                ...
                // Close existing channel if exists / 如果存在则关闭现有 channel
                if (_channel != null)
                {
                    try
                    {
                    ...
                    {
                        _logger.LogWarning(ex, "[RabbitMQMessageQueueService] 关闭旧 channel 时发生错误");
                    }
                    _channel = null;
                }

                // Close existing connection if exists / 如果存在则关闭现有连接
                if (_connection != null)
                {
                    try
                    {
                    ...
                    {
                        _logger.LogWarning(ex, "[RabbitMQMessageQueueService] 关闭旧连接时发生错误");
                    }
                    _connection = null;
                }
                ...
                // Verify connection and channel are truly ready / 验证连接和 channel 真正就绪
                if (_connection == null || !_connection.IsOpen)
                {
                    throw new InvalidOperationException("RabbitMQ connection failed to establish");
                }
                if (_channel == null || !_channel.IsOpen)
                {
                    throw new InvalidOperationException("RabbitMQ channel failed to establish");
                }
                
                // Test channel with a simple operation to ensure it's truly ready / 使用简单操作测试 channel 确保真正就绪
                try
                {
                    // Try to get channel number as a test / 尝试获取 channel 编号作为测试
                    var channelNumber = _channel.ChannelNumber;
                    _logger.LogWarning($"[RabbitMQMessageQueueService] RabbitMQ 连接成功 - ConnectionIsOpen: {_connection.IsOpen}, ChannelIsOpen: {_channel.IsOpen}, ChannelNumber: {channelNumber}");
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "[RabbitMQMessageQueueService] Channel 测试失败，连接可能不稳定");
                    throw new InvalidOperationException("RabbitMQ channel is not ready", ex);
                }
                ...
                // Final verification: ensure channel is still open after operations / 最终验证：确保操作后 channel 仍然打开
                if (_channel == null || !_channel.IsOpen)
                {
                    throw new InvalidOperationException("RabbitMQ channel closed during initialization");
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task DeclareExchangeAsync(string exchangeName, string exchangeType, bool durable = true)
        {
            ...
            // Store exchange info for reconnection / 保存交换机信息以便重新连接
            _declaredExchanges[exchangeName] = (exchangeType, durable);
        ...
        public async Task<string> DeclareQueueAsync(string queueName, bool durable = false, bool exclusive = false, bool autoDelete = true)
        {
            ...
            // Store queue info for reconnection / 保存队列信息以便重新连接
        ...
        public async Task BindQueueAsync(string queueName, string exchangeName, string routingKey)
        {
            ...
            // Store binding info for reconnection / 保存绑定信息以便重新连接
            if (!_queueBindings.ContainsKey(queueName))
            {
                _queueBindings[queueName] = new List<(string, string)>();
            }
        ...
        {
            // Prepare message properties / 准备消息属性
            ...
            // Retry publishing with connection recovery / 重试发布并恢复连接
            int publishRetries = 0;
            int maxPublishRetries = 5; // Increase retries / 增加重试次数
            
            while (publishRetries < maxPublishRetries)
            {
                try
                {
                    // Ensure connection and channel are open / 确保连接和 channel 已打开
                    // Check first without lock / 先无锁检查
                    bool needsReconnect = _connection == null || !_connection.IsOpen || _channel == null || !_channel.IsOpen;
                    
                    if (needsReconnect)
                    {
                        // Set reconnecting flag / 设置重连标志
                        lock (_reconnectLock)
                        {
                            if (!_isReconnecting)
                            {
                        ...
                        // Wait for reconnection / 等待重连
                        await EnsureConnectedAsync();
                    }
                    else
                    {
                        // Double-check with lock before publishing / 发布前使用锁再次检查
                        lock (_reconnectLock)
                        {
                            if (_connection == null || !_connection.IsOpen || _channel == null || !_channel.IsOpen)
                            {
                                // Channel closed between checks, need reconnect / channel 在检查之间关闭，需要重连
                                _isReconnecting = true;
                            }
                        }
                    ...
                    // Verify channel is still open before publishing / 发布前验证 channel 仍然打开
                    if (_channel == null || !_channel.IsOpen)
                    {
                        throw new InvalidOperationException($"Channel is not ready - ChannelIsNull: {_channel == null}, ChannelIsOpen: {_channel?.IsOpen ?? false}");
                    }
                    
                    // Additional verification: check connection is also open / 额外验证：检查连接也打开
                    if (_connection == null || !_connection.IsOpen)
                    {
                        throw new InvalidOperationException($"Connection is not ready - ConnectionIsNull: {_connection == null}, ConnectionIsOpen: {_connection?.IsOpen ?? false}");
                    }
                    
                    _logger.LogTrace($"[RabbitMQMessageQueueService] 发布消息 - ExchangeName: {exchangeName}, RoutingKey: {routingKey}, MessageId: {properties?.MessageId}, MessageSize: {message.Length} bytes, ChannelIsOpen: {_channel.IsOpen}, Attempt: {publishRetries + 1}");
                    ...
                    _logger.LogTrace($"[RabbitMQMessageQueueService] 消息发布成功 - ExchangeName: {exchangeName}, RoutingKey: {routingKey}, MessageId: {properties?.MessageId}");
                    return; // Success, exit retry loop / 成功，退出重试循环
                }
                catch (Exception ex)
                {
                    publishRetries++;
                    
                    // Check if it's a channel closed error / 检查是否是 channel 关闭错误
                    bool isChannelError = ex is InvalidOperationException || 
                    ...
                    {
                        _logger.LogError(ex, $"[RabbitMQMessageQueueService] 消息发布失败，已达到最大重试次数 ({maxPublishRetries}) - ExchangeName: {exchangeName}, RoutingKey: {routingKey}, MessageId: {properties?.MessageId}, Error: {ex.Message}");
                        throw new InvalidOperationException($"RabbitMQ channel is not open after {maxPublishRetries} retries", ex);
                    }
                    
                    if (isChannelError)
                    {
                        _logger.LogWarning($"[RabbitMQMessageQueueService] Channel 错误，尝试重新连接并重试 ({publishRetries}/{maxPublishRetries}) - ExchangeName: {exchangeName}, RoutingKey: {routingKey}, Error: {ex.Message}");
                        
                        // Reset connection state and retry / 重置连接状态并重试
                        lock (_reconnectLock)
                        {
                            _isReconnecting = false;
                            _connection = null;
                        ...
                        await Task.Delay(200 * publishRetries); // Exponential backoff / 指数退避
                    }
                    else
                    {
                        // Non-channel error, throw immediately / 非 channel 错误，立即抛出
                        _logger.LogError(ex, $"[RabbitMQMessageQueueService] 消息发布失败（非 channel 错误）- ExchangeName: {exchangeName}, RoutingKey: {routingKey}, MessageId: {properties?.MessageId}, Error: {ex.Message}");
                        throw;
                    }
                }
            }
            
            // Should not reach here, but just in case / 不应该到达这里，但以防万一
            throw new InvalidOperationException($"Failed to publish message after {maxPublishRetries} retries");
        }
