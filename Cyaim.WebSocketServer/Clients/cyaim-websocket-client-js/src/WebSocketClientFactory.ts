...
export class WebSocketClientFactory {
   ...
   * Constructor / 构造函数
   */
  constructor(serverBaseUrl: string, channel: string = '/ws', options?: WebSocketClientOptions) {
  ...
  async getEndpoints(): Promise<WebSocketEndpointInfo[]> {
    ...
    try {
      ...
      const apiResponse: ApiResponse<WebSocketEndpointInfo[]> = await response.json();
      ...
      throw new Error(apiResponse.error || 'Failed to fetch endpoints');
    } catch (error) {
      throw new Error(`Error fetching WebSocket endpoints from server: ${error}`);
    }
  }
   ...
   */
  async createClient<T extends Record<string, (...args: any[]) => Promise<any>>>(
    interfaceDefinition: T
  ): Promise<T> {
    const wsUri = this.serverBaseUrl.replace(/^http/, 'ws');
    const client = new WebSocketClient(wsUri, this.channel);
   ...
   */
  private createProxy<T extends Record<string, (...args: any[]) => Promise<any>>>(
    ...
    for (const [methodName, method] of Object.entries(interfaceDefinition)) {
      ...
      if (endpointTarget) {
        if (endpoints) {
          endpoint = endpoints.find(ep =>
            ep.target.toLowerCase() === endpointTarget.toLowerCase() ||
            ep.methodPath.toLowerCase() === endpointTarget.toLowerCase() ||
            ep.fullName.toLowerCase() === endpointTarget.toLowerCase()
          ) || null;
        } else {
          // Lazy loading: create temporary endpoint / 延迟加载：创建临时端点
          endpoint = { target: endpointTarget } as WebSocketEndpointInfo;
        }
      } else if (endpoints) {
        // Try to find by method name / 尝试通过方法名查找
        endpoint = endpoints.find(ep =>
          ep.action.toLowerCase() === methodName.toLowerCase() ||
          ep.methodPath.toLowerCase() === methodName.toLowerCase()
        ) || null;
      }
      ...
      proxy[methodName] = async (...args: any[]) => {
        ...
        if (!endpoint && this.options.lazyLoadEndpoints) {
          const allEndpoints = await this.getEndpoints();
          const endpointTarget = (method as any).__endpointTarget;
          if (endpointTarget) {
        ...
        if (!endpoint) {
          ...
          if (this.options.throwOnEndpointNotFound) {
        ...
        if (args.length === 1) {
          ...
          const paramNames = (method as any).__paramNames || [];
          requestBody = {};
          for (let i = 0; i < args.length && i < paramNames.length; i++) {
            requestBody[paramNames[i]] = args[i];
          }
        }

        await client.connect();
        return await client.sendRequest<any, any>(endpoint.target, requestBody);
      };
    }
