...
`HybridClusterTransport` 支持将连接路由信息存储到 Redis，实现快速查询而无需广播：

#### 工作原理

1. **连接注册时**：
   - 将连接路由信息存储到 Redis：`websocket:cluster:connections:{connectionId}` -> `{nodeId}`
   - 存储连接元数据：`websocket:cluster:connection:metadata:{connectionId}` -> JSON
   - 设置 24 小时过期时间

2. **连接查询时**：
   - 直接从 Redis 读取连接路由信息，无需广播查询
   - 如果找到，更新本地路由表缓存

3. **连接注销时**：
   - 从 Redis 删除连接路由信息和元数据

4. **自动续期**：
   - 每 12 小时自动刷新所有本地活跃连接的路由信息
   - 重新设置 24 小时过期时间，确保活跃连接的路由信息不会过期

#### 优势

- ✅ **快速查询** - 直接从 Redis 读取，延迟低
- ✅ **无需广播** - 减少 RabbitMQ 消息量
- ✅ **自动同步** - 所有节点共享同一 Redis，路由表自动同步
- ✅ **自动续期** - 活跃连接的路由信息不会因过期而丢失

#### 实现细节

连接路由存储功能通过 `IClusterTransport` 接口的可选方法实现：

- `StoreConnectionRouteAsync` - 存储连接路由信息
- `GetConnectionRouteAsync` - 获取连接路由信息
- `RemoveConnectionRouteAsync` - 删除连接路由信息
- `RefreshConnectionRouteAsync` - 刷新连接路由过期时间

只有 `HybridClusterTransport` 实现了这些方法，其他传输层（WebSocket、Redis Pub/Sub、RabbitMQ）不支持此功能，会自动回退到广播方式。

#### 配置

连接路由存储功能是**自动启用**的，无需任何配置。当使用 `HybridClusterTransport` 时：

- 连接注册时自动存储到 Redis
- 连接查询时自动从 Redis 读取
- 每 12 小时自动刷新本地连接的路由信息
...
系统会定期（每 12 小时）检查所有本地活跃连接，并刷新它们在 Redis 中的过期时间：

- 只刷新本地连接（`nodeId == 当前节点ID`）
- 验证连接是否仍然存在于本地路由表中
- 如果刷新失败，尝试重新存储路由信息
- 记录刷新成功和失败的连接数量

这确保了即使连接存在超过 24 小时，路由信息也不会因过期而丢失。
...
2. **消息ID去重** - ~~使用消息ID防止重复处理同一消息~~（已移除，按用户要求）
3. **自动清理** - ~~定期清理过期的消息ID记录（每5分钟清理一次，保留10分钟内的记录）~~（已移除，按用户要求）

> **注意**：消息去重机制已按用户要求移除，系统不再过滤重复的消息ID。

### 工作原理

```text
消息到达 → 检查是否来自自己 → 检查目标节点 → 处理消息
```

#### 目标节点检查

- 如果消息的 `ToNodeId` 不为空且不等于当前节点ID，消息会被忽略
- 如果 `ToNodeId` 为 `null`（广播消息），所有节点都会处理
- 如果 `ToNodeId` 等于当前节点ID，消息会被处理

> **注意**：消息ID去重机制已移除，系统不再检查消息ID是否已处理过。
...
// 结果：
// - 所有重复消息：都会正常处理 ✅
// 注意：如果业务逻辑需要去重，请在应用层实现
```

### 配置说明

目标节点检查是**自动启用**的，无需任何配置。系统会：

- 自动检查目标节点
- 自动忽略发送给其他节点的消息

> **注意**：消息ID去重机制已移除，系统不再缓存和检查消息ID。
...
- ⚠️ **注意**：消息去重机制已移除，系统不再过滤重复的消息ID
- 如果业务逻辑需要去重，请在应用层实现：
  - 在消息处理逻辑中检查消息ID
  - 使用分布式锁或缓存记录已处理的消息ID
  - 根据业务需求实现去重逻辑
- 检查消息的 `ToNodeId` 是否正确设置
- 查看日志中是否有 "Ignoring message - target node is" 的提示（这是正常的，表示消息被正确路由）
