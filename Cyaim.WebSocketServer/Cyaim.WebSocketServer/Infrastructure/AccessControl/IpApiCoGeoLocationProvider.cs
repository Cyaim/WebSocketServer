...
{
    /// <summary>
    /// Geographic location provider using ipapi.co (free tier)
    /// 使用 ipapi.co（免费版）的地理位置提供者
    /// Rate limit: 1,000 requests/day for free tier / 免费版限制：每天 1,000 次请求
    /// </summary>
    public class IpApiCoGeoLocationProvider : IGeoLocationProvider
    {
        private readonly ILogger<IpApiCoGeoLocationProvider> _logger;
        ...
        /// <param name="apiUrl">API URL (default: https://ipapi.co/{ip}/json/) / API URL（默认：https://ipapi.co/{ip}/json/）</param>
        public IpApiCoGeoLocationProvider(
            ILogger<IpApiCoGeoLocationProvider> logger,
            ...
            string apiUrl = null)
        {
            ...
            _apiKey = apiKey;
            _apiUrl = apiUrl ?? "https://ipapi.co/{ip}/json/";
        }
